...
<body>
			...
			<div class="panel rt">
				<form id="form-register">
					<div class="txt">
						<p>
							新用户注册 <span> <a
								href="${pageContext.request.contextPath}/web/view/login">直接登录</a>
							</span>
						</p>
					</div>
	...
	<script>
		$('#bt-register')
				.click(
						function() {
							if($('[name=upwdconfirm]').val()!=$('[name=upwd]').val()){
								upwdconfirm.nextElementSibling.innerHTML = '密码不符,请重新输入';
							 	upwdconfirm.setCustomValidity('密码不符,请重新输入');
								upwdconfirm.nextElementSibling.className = 'msg-error';
								}
							var lengths = 0;
							$('.form-group').each(
									function() {
							...
							if (lengths == 5) {
								...
								$
										.ajax({
											url : '${pageContext.request.contextPath}/web/action/register',
											data : data,
											type : 'POST',
											dataType : 'json',
	...
	<script>
		...
		uname.onblur = function() {
			if (this.validity.valueMissing) {
			...
			} else if (/* this.validity.tooShort */$(this).val().length < 6) {
				this.nextElementSibling.innerHTML = '用户名不能少于6位';
				this.nextElementSibling.className = 'msg-error';
				this.setCustomValidity('用户名不能少于6位');
			} else {
				this.nextElementSibling.innerHTML = '正在查询中...';
				this.nextElementSibling.className = 'msg-default';
				this.setCustomValidity('正在查询中...');
				...
				$
						.ajax({
							url : '${pageContext.request.contextPath}/web/action/checkUserName',
							data : {
								"username" : data
							},
							type : 'post',
							...
							success : function(msg) {
								if (msg.code == 1) {
									element.nextElementSibling.innerHTML = '用户名可以使用';
									element.nextElementSibling.className = 'msg-success';
									element.setCustomValidity('用户名可以使用');
								} else if (msg.code == 0) {
									element.nextElementSibling.innerHTML = '用户名已被占用';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('用户名已被占用');
								} else if (msg.code == -1) {
									element.nextElementSibling.innerHTML = '无法查询用户名';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('无法查询用户名');
								}
							},
							error : function() {
								element.nextElementSibling.innerHTML = "请求服务器失败,请稍后重试";
								element.nextElementSibling.className = 'msg-error';
								element.setCustomValidity('请求服务器失败,请稍后重试');
							}
						});
			}
		}
		...
		upwdconfirm.onblur = function() {
			if (this.validity.valueMissing) {
				...
				this.nextElementSibling.innerHTML = '密码不符,请重新输入';
				this.nextElementSibling.className = 'msg-error';
				this.setCustomValidity('密码不符,请重新输入');
			} else {
				this.nextElementSibling.innerHTML = '密码格式正确';
				this.nextElementSibling.className = 'msg-success';
				this.setCustomValidity('');
		...
		email.onblur = function() {
			if (this.validity.valueMissing) {
				...
				$
						.ajax({
							url : '${pageContext.request.contextPath}/web/action/checkEmail',
							data : {
								"email" : data
							},
							type : 'post',
							...
							success : function(msg) {
								if (msg.code == 1) {
									element.nextElementSibling.innerHTML = '该邮箱可以使用';
									element.nextElementSibling.className = 'msg-success';
									element.setCustomValidity('该邮箱可以使用');
								} else if (msg.code == 0) {
									element.nextElementSibling.innerHTML = '该邮箱已被占用';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('该邮箱已被占用');
								} else if (msg.code == -1) {
									element.nextElementSibling.innerHTML = '无法查询该邮箱';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('无法查询该邮箱');
								}
							},
							error : function() {
								element.nextElementSibling.innerHTML = "请求服务器失败,请稍后重试";
		...
		phone.onblur = function() {
			if (this.validity.valueMissing) {
				...
				$
						.ajax({
							url : '${pageContext.request.contextPath}/web/action/checkPhone',
							data : {
								"phone" : data
							},
							type : 'post',
							...
							success : function(msg) {
								if (msg.code == 1) {
									element.nextElementSibling.innerHTML = '手机号可以使用';
									element.nextElementSibling.className = 'msg-success';
									element.setCustomValidity('手机号可以使用');
								} else if (msg.code == 0) {
									element.nextElementSibling.innerHTML = '手机号已被占用';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('手机号已被占用');
								} else if (msg.code == -1) {
									element.nextElementSibling.innerHTML = '无法查询该手机号';
									element.nextElementSibling.className = 'msg-error';
									element.setCustomValidity('无法查询手机号');
								}
							},
							error : function() {
								element.nextElementSibling.innerHTML = "请求服务器失败,请稍后重试";
