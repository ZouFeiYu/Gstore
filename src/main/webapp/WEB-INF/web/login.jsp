...
<body>
	...
	<div id="container">
		<div id="cover" class="rt">
			<form id="login-form" method="post" name="form1">
				<div class="txt">
					<p>
						登录学子商城<span><a href="register.page">新用户注册</a></span>
					</p>
					<div class="text">
	...
	<script>
		...
		$(function() {
			$.ajax({
				url : '${pageContext.request.contextPath}/web/action/askRegisterInfo',
				type : 'POST',
				dataType : 'json',
				success : function(msg) {
					if (msg.code == 1) {
						$("#username").val(msg.data.uname);
						$("#password").val(msg.data.upwd);
					}
				},
				error : function() {
				}
			...
			$.ajax({
				...
				success : function(msg) {
					if (msg.code == 1) {
						...
						$("#showResult").text("该用户名可以登录");
						$("#showResult").css("color", "white");
					}
				},
				error : function() {
					$("#showResult").text("服务器繁忙...");
					$("#showResult").css("color", "red");
				}
			});
		});
	...
	<script>
		$('#bt-login').click(function() {
			...
			var rmbUser = $("#ck_rmbUser").prop("checked")
			$.ajax({
				url : '${pageContext.request.contextPath}/web/action/login',
				type : 'POST',
				data : inputData + "&rmb=" + rmbUser,
				dataType : 'json',
				success : function(msg) {
					if (msg.code == 1) {
						Save();//调用js保存用户名、密码到cookie
						alert('登录成功');
						$(location).attr('href','${pageContext.request.contextPath}/web/'+ msg.data.url);
					} else {
						$("#showResult").text("用户名或密码不正确");
						$("#showResult").css("color", "red");
					}
				},
				error : function() {
