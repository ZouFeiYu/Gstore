...
<div id="container" class="clear">
    ...
    <div class="adress_choice">
        <p>收货人信息<span class="rt" id="choose">新增收货地址</span></p>
        	...
        	<c:if test="${addr.state==1 }">
        	<div id="${addr.id }" class="base_select addresId1">
        	</c:if>
        	<c:if test="${addr.state==0 }">
        	<div id="${addr.id }" class="base addresId2">
        	</c:if>
            ...
            <i class="user_address">
              ${addr.provinceCode } ${addr.citieCode } ${addr.areaCode } ${addr.detailed }
            </i>
    ...
    <form name="" method="post" action="#">
        <div class="product_confirm">
            ...
            <ul class="item_detail">
                <li class="p_info">
                    <b><img src="${pageContext.request.contextPath}${item.image}"/></b>
                ...
                <li class="p_count">X<span>${item.count}</span></li>
...
<script>
    $(".go_pay").click(function () {
        var addrId;
    	$(".adress_choice div").each(function(){
			if($(this).hasClass("base_select")){
				addrId=$(this).attr("id");
			}
		});
		$.ajax({
			url:'${pageContext.request.contextPath}/web/action/order/confirm',
			type:'POST',
			data:{'addrId':addrId},
			dataType:'JSON',
			success:function(msg){
				if(msg.code==1){
					alert("下单成功,请结算");
					$(location).attr("href","${pageContext.request.contextPath}/web/view/payment")
				}else{
					alert("下单失败");
				}
			},
...
<script>
	...
	$(".user_site").click(function(){
		//$(this).parent().attr("class","base_select addresId1");
		//$(this).parent().siblings().attr("class","base addresId2");
		$(".adress_choice div").attr("class","base addresId2");
		$(this).parent().attr("class","base_select addresId1");
		//$('#more').removeClass("addresId2");
		$(this).hide();
		$(".adress_choice").children().each(function(){
			$(this).show();
		}); 
	})
</script>
